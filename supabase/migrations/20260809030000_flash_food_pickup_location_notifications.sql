-- A flash-food campaign can hand off each delivery location at a different
-- time. Keep that schedule separate from the campaign's former one-time
-- pickup_ready_at field so no location is notified early.
create table if not exists public.flash_food_pickup_notices (
  campaign_id uuid not null references public.flash_food_campaigns(id) on delete cascade,
  pickup_location text not null check (pickup_location in ('明德樓', '據德樓', '蘊德樓', '機車停車場')),
  pickup_at timestamptz not null,
  notified_at timestamptz not null default now(),
  notified_by uuid not null references auth.users(id) on delete restrict,
  updated_at timestamptz not null default now(),
  primary key (campaign_id, pickup_location)
);

create index if not exists flash_food_pickup_notices_campaign_idx
  on public.flash_food_pickup_notices (campaign_id, pickup_at);

alter table public.flash_food_pickup_notices enable row level security;

drop policy if exists "admins manage flash food pickup notices" on public.flash_food_pickup_notices;
create policy "admins manage flash food pickup notices"
  on public.flash_food_pickup_notices
  for all
  to authenticated
  using (public.is_admin_user())
  with check (public.is_admin_user());

create or replace function public.touch_flash_food_pickup_notice_updated_at()
returns trigger
language plpgsql
as $$
begin
...
drop trigger if exists touch_flash_food_pickup_notice_updated_at on public.flash_food_pickup_notices;
create trigger touch_flash_food_pickup_notice_updated_at
before update on public.flash_food_pickup_notices
for each row execute function public.touch_flash_food_pickup_notice_updated_at();
