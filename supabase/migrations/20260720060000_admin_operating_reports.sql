-- Keep the shipping fee on the order so financial reports do not change when the fee changes later.
...
begin
  ...
  from (
    select
      ...
      jsonb_build_object(
        ...
        'amount', coalesce(sum(o.shipping_amount), 0),
        'orders', count(o.id)
      ) as payload
  ...
  select jsonb_build_object(
    ...
    'earned_shipping_amount', coalesce(sum(o.shipping_amount) filter (
    ...
    'estimated_shipping_amount', coalesce(sum(o.shipping_amount) filter (
