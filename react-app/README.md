...
- 手機版 `AdminOrderDrawer.jsx` 在待取貨且實際總額不同於原報價時，提供「補傳金額更正通知」。下單完成、訂金確認、金額更正與交貨通知是四個獨立事件；下單完成由 `create-order` 依建立當下狀態自動建立。相關 `create-order`、`line-notify` 與 `20260812010000_order_created_line_notifications.sql` 必須一起部署。
