...
- LINE 官方帳號綁定、通知開關、明確的訂金／金額更正／交貨推播、佇列診斷與失敗重試。
...
LINE 通知不再綁定一般狀態異動：確認訂金後只送一則「訂單總額／訂金金額」通知；`ready_pickup` 是內部「已加入待交貨清單」的里程碑，不立即發送。若採購完成時實際總額有異動，管理者可在手機版訂單明細補傳一則「原訂單總額／更正後總額／訂金」通知。管理者在訂單管理選定交貨地點與交貨時間後，才對該地點 `ready_pickup` 會員群發一次交貨通知；有價格異動時，通知依序帶入調整後訂單總額、訂金與尾款快照。
...
截至 2026-08-11，遠端 migration 履歷已同步到 `20260811020000_explicit_notification_event_types.sql`，且 `create-order`、`line-notify` 已部署。通知佇列允許訂金確認與金額更正兩種明確事件，新訂單不再建立舊的初始狀態通知。
